...
-- ~~EIGHT SINCE ROUND 25.~~ SEVEN AGAIN SINCE ROUND 27, item 6b: pay-first
-- removed the state `is_pending` described, so the column was always false and
-- the cleanup migration drops it. Restated in full rather than edited in
-- place, so a column cannot arrive or leave without somebody typing it here.
select pg_temp.ok(
