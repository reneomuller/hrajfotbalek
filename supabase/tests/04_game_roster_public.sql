...
-- ~~EIGHT SINCE ROUND 25~~ — SEVEN AGAIN SINCE ROUND 27, item 6b. `is_pending`
-- was round 25's answer to a seat held by a checkout in progress; pay-first
-- removed the state, so the column had been constantly false and the cleanup
-- migration drops it. The list is restated in full rather than patched, which
-- is the whole point of enumerating: a column cannot arrive OR LEAVE without
-- somebody typing it here. The withholding assertion below is unchanged and is
