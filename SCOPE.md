...
| ~~Public player profile~~ | **LIFTED, round 14 (R32).** Four things and nothing else: picture, banner, the three stats, badges. The scope is enforced by a six-column composite return type rather than by the page |
| In-app notifications surface | There is no notification store; email is the channel |
| **Per-game pitch names** — typed at creation, reused from a dropdown | Written as migration 41 and NOT applied. `pitch_name` lives on `venues`, so it is one name shared by every game there: storing a typed name on the venue would rewrite the pitch of every past game at that ground. The migration adds `games.pitch_name` plus a `pitch_name_suggestions` view. One question is open — whether "save this pitch" is a real flag or every typed name is simply remembered — and the UI is unbuilt until the owner applies |
