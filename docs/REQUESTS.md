...
| 203 | **Apply round 27's two migrations** | **`SHIPPED` — and the OWNER applied them.** **Probed on production 2026-09-17 by the OBJECTS, not by the filenames or by last round's word** — which is the whole reason this ledger re-verifies: it was reported blocked and it was not. `add_guests_with_credit`, `open_add_guests_checkout` and `can_add_guests` are all in `pg_proc`, and `app_capabilities()` returns `addGuestsAfterBooking: true` and `addGuestsConfirmation: true`. Superseded in practice by row 226, which said the same thing about the same files; both are closed by the same probe |
| 215 | **Apply round 28's two migrations** | **`SHIPPED` — and the OWNER applied them.** **Probed on production 2026-09-17 by the OBJECTS, not by the filenames or by last round's word** — which is the whole reason this ledger re-verifies: it was reported blocked and it was not. `credit_ledger.note` is a column and `app_capabilities()` returns `publicProfileScope: true` and `creditLedgerNote: true` |
| 221 | **Apply `20260909120000_auto_settle.sql`** | **`SHIPPED` — and the OWNER applied it, backfill included.** **Probed on production 2026-09-17 by the OBJECTS, not by the filenames or by last round's word** — which is the whole reason this ledger re-verifies: it was reported blocked and it was not. `mark_game_played` and `advance_played_games` are in `pg_proc` and `app_capabilities()` returns `autoSettle: true` and `playedSweep: true`. **And the backfill visibly ran**: of 53 games on production, **44 are `settled`**, 7 are `cancelled` and 2 are `published` — there is no `played` backlog for the nightly sweep to report, which is the state this migration existed to produce |
| 226 | **Re-apply commands for rounds 27 and 28 (supersedes rows 203 and 215)** | **`SHIPPED` — all four applied.** **Probed on production 2026-09-17 by the OBJECTS, not by the filenames or by last round's word** — which is the whole reason this ledger re-verifies: it was reported blocked and it was not. Every object the four files create is present and every flag they set is true. This row existed because the four had been reported blocked twice; they are not |
...
| 244 | *The edge the owner expected to be empty.* **Three of four production wallets do NOT divide into whole credits** | **`CLOSED round-36` — re-probed rather than echoed, and it is empty now.** The three ragged balances (4,460 · 110 · 50 CZK) are gone: the whole `credit_ledger` on production sums to **180 CZK across one player**, which is exactly one credit and divides cleanly. Round 35 v2 zeroed the wallets and v5 zeroed them again; this is what has accumulated since. **`adminWallet`'s ragged path is kept** — it is arithmetic that must be right the first time somebody is granted an odd amount by hand, and `lib/admin/__tests__/credits.test.ts` says so in its own comment |
...
| 261 | *Found re-verifying row 187.* **`checkout_sessions` holds 28 LIVE Stripe sessions and NOT ONE has settled** | `EXPLAINED round-36 — CLOSED BY THE OWNER. They were his own pre-launch tests.` The row named two readings this side could not tell apart, and the alarming one is ruled out: **no unsettled customer money, and no seats paid for and never held.** An `open` checkout session is inert — it holds nothing, charges nothing and expires on its own — so there is no cleanup owed and nothing to reconcile. **The row is kept rather than deleted** because the SHAPE it describes is real: 28 created-and-registered sessions with no settlement is exactly what the round-27 outage looked like, and the next time that pattern appears the question has to be asked again rather than answered from this row |
...
| 275 | **Apply round 34's migration** | **`SHIPPED` — and the OWNER applied it.** **Probed on production 2026-09-17 by the OBJECTS, not by the filenames or by last round's word** — which is the whole reason this ledger re-verifies: it was reported blocked and it was not. `cancel_guests` is in `pg_proc` and `app_capabilities()` returns `cancelGuests: true`, so the remove-guests control is live rather than gated off |
| 291 | **Apply round 35's migration** | **`SHIPPED` — and the OWNER applied it.** **Probed on production 2026-09-17 by the OBJECTS, not by the filenames or by last round's word** — which is the whole reason this ledger re-verifies: it was reported blocked and it was not. `credit_seat_price_czk` is in `pg_proc` and `app_capabilities()` returns `creditsAreSeats: true`. Round 35 v5's own migration is applied too (`credit_seat_minutes`, `price_for_duration`, `durationPricing: true`), which row 318 already recorded |
...
| 312 | *The flag item 2 asked for.* **One 120-minute game exists, and it is priced at 180 pending a ruling** | `MOOTED round-36 — the organizer types the price, so there is nothing left to rule on.` The question only existed because a mapping had to answer for every possible length; 120 minutes now PREFILLS 180 and the organizer overwrites it in the same box as every other game. **The pre-pick survives as a default and is asserted as one** in `lib/games/__tests__/price.test.ts` — what ends is the obligation to invent a tier for a length nobody has scheduled twice |
...
| 320 | *What could not be verified live, stated rather than implied.* **There is no published 60-minute game on production** | `OPEN — informational, and RE-PROBED on 2026-09-17 rather than carried forward: still true.` The only published games are **two 90-minute ones, both at 180**. The 34 sixty-minute-or-null games are all `settled` or `cancelled`. The short game's online-only path is proved in the SQL drill and in `e2e/round35v5.spec.ts`; **the first short game the owner publishes is still the real check**, and it needs nothing from me |
| 321 | **(R36-1) Apple Pay renders twice in the embedded checkout — and the duplicate is Stripe's own layout** | `ANSWERED round-36 BY ELIMINATION, WITH THE EVIDENCE — and the minimizing config shipped.` Three layers could produce a second wallet and two are ruled out by inspection. **(a) OUR PAGE SHELL**: `EmbeddedCheckoutFrame.tsx` renders exactly one `<EmbeddedCheckoutProvider><EmbeddedCheckout /></EmbeddedCheckoutProvider>`, and there is no express-checkout element anywhere in the repo — nothing of ours draws a wallet button at all. **(b) OUR SESSION CONFIG**: a Checkout Session has NO parameter that can surface a wallet twice. `SessionCreateParams.WalletOptions` in the installed SDK contains exactly one member, `link`; `apple_pay` is not a valid `PaymentMethodType`; and `express_checkout`, `wallet_placement` and `payment_layout` have **zero matches in the 5,137-line declaration file**. **So it is (c), Stripe's internal embedded layout**, which places the wallet row both above the form and beside the pay button and is not addressable from session parameters. **Said plainly rather than shrugged at**, and the config that minimizes the wallet row is shipped: `wallet_options: { link: { display: "never" } }` |
| 322 | *What the item asked for and this environment cannot give.* **The visual reproduction could not be run** | `STATED round-36 rather than faked.` Creating a Checkout Session needs the live secret key, and `vercel env pull` returns `STRIPE_SECRET_KEY="[SENSITIVE]"` — Vercel redacts it by design, so no session can be created outside production and there is no embedded checkout to photograph here. **The evidence in row 321 is therefore static rather than visual**, which is stated because the difference matters: it proves what our code and the SDK can and cannot do, not what Stripe's iframe drew on the owner's phone |
| 323 | *Pinned as an ABSENCE, which is the only way this one can be watched.* **A future Stripe SDK that adds a wallet knob must reopen row 321** | `SHIPPED round-36`. `lib/payments/__tests__/walletOptions.test.ts` reads the installed `Sessions.d.ts` and asserts that `WalletOptions` mentions no `apple_pay` and no `google_pay`, and that no express-checkout or layout parameter exists. **The conclusion "Stripe gives us no knob" is true of a VERSION, not of the world** — so the day Stripe ships one, the test fails and names the row to revisit rather than the answer standing unexamined |
| 324 | **(R36-2) The game price is a typed field again; the duration only prefills it** | `SHIPPED round-36`, migration written and **handed over** — `20260918100000_price_is_typed.sql`. A reversal of row 311 by the owner's ruling: 60 minutes prefills 150, 90 prefills 180, anything else prefills 180, and the organizer overwrites any of them with any positive whole number. Both writers store what they are sent; the card path charges the typed price exactly, server-side, as it already did. **Validation tightened from `>= 0` to `>= 1`**: zero was unreachable while the field was derived, and a game priced at nothing is a booking Stripe refuses to charge for — the failure belongs on the organizer at the form, not on a player at checkout |
| 325 | *The owner's call, noted as the item asked.* **Changing the duration OVERWRITES a customised price** | `SHIPPED round-36, and asserted so it cannot drift into a surprise.` The alternative is a field that remembers whether its value was chosen or suggested and silently declines to update — a rule that is invisible on screen and that nobody can predict from outside. The organizer retypes after changing the length, which is one visible step instead of an invisible one. `e2e/round36.spec.ts` pins it, so a later round adding "sticky" behaviour has to decide it deliberately |
| 326 | *What round 36 had to keep apart.* **Credit eligibility keys on the DURATION and reads no price at all** | `SHIPPED round-36`. The two were briefly the same question. Every fixture in `e2e/round36.spec.ts` and in the `duration_pricing` drill is now priced ACROSS the old mapping — a 90-minute game at 150, a 60-minute one at exactly the credit nominal of 180 — because a fixture priced ON the mapping cannot tell a product that reads the length from one that reads the price. The 90-minute game takes one flat credit however cheap it is; the 60-minute one stays online-only even when its price matches a credit to the crown |
| 327 | *The inversion, stated.* **Round 35 v5's "0 games off-price" assertion is gone, and what replaced it** | `SHIPPED round-36`. A census over `games.price_czk = price_for_duration(duration_minutes)` was a LAW asserted over a mapping that is now a DEFAULT: it would fail the first time an organizer typed a price, which is the feature. The `duration_pricing` suite now asserts the properties that actually hold — the writers store what they are handed, the prefill mapping still returns 150/180, no credit was ever applied to a game that is not 90 minutes, and every credit that was applied came in whole 180 CZK seats. `lib/pass/__tests__/seatPrice.test.ts` reads the form's source for a writable, state-bound input and the `>= 1` validator |
| 328 | *Found while inverting row 327, and it means round 35 v5's edit half never shipped.* **The derive went into `admin_update_game`; every edit in `app/` calls `admin_update_game_v2`** | `FIXED round-36 by the item that reversed it.` The seven-argument legacy function got round 35 v5's substitution and the thirteen-argument overload the admin form actually uses was never touched — so from round 35 v5 to round 36 the edit form stored the typed price the whole time while the drill said prices were derived. **Nothing was wrong on production** because round 36 wants exactly that behaviour, but the drill was passing for a reason unconnected to the product. The migration now pins `admin_update_game_v2` as an ABSENCE — it must not contain `price_for_duration` — and the e2e drives the real form instead of an RPC nothing calls |
| 329 | *A false pass of the same family CLAUDE.md already records.* **`X or true` is folded away, so the call inside it never ran** | `FIXED round-36`. The `duration_pricing` drill wrapped its edit as `(select admin_update_game(…)) is not null or true` to tolerate a return value, and Postgres constant-folds `or true` without evaluating the left side. The update never executed; the price the assertion "confirmed" was the one the CREATE had already written. **Same shape as the `count(*)` pruning trap**, wearing a different hat: a probe that passes without running the thing it probes. The call is now its own statement |
| 330 | *Where the fixtures had to move with the ruling, again.* **Eight specs asserted a price the scaffold used to override** | `SHIPPED round-36`. `createScratchGame` defaults to 200 CZK, and round 35 v5's derive quietly rewrote that to 150 or 180 — so specs asserting `booking.price_czk = 3 x 180` passed on a number nobody had asked for. With the typed price honoured the default is stored again, and every spec that asserts a price now states it: `round34.spec.ts` names the credit nominal, `round35v5.spec.ts` names the mapping's own prices. The scaffold still resolves a LENGTH from the price when none is given, because what that resolution really selects is credit eligibility |

| 331 | **Apply round 36's migration** | **`SHIPPED` — and the OWNER applied it, between round 36's report and round 37's.** Re-probed on production 2026-09-17 rather than echoed: `admin_create_game_v2` now carries the literal comment `-- ROUND 36: the organizer types the price; the length only prefills it.` and the statement `coalesce(p_price_czk, public.price_for_duration(p_duration_minutes))`, and `admin_update_game_v2` still contains no `price_for_duration` — the absence the migration pins. **A typed price is now stored by both writers on production**, which is what round 36 shipped the front end for. All 27 capability flags are true |
| 332 | *Found by re-verifying rather than echoing — six rows at once.* **Rows 203, 215, 221, 226, 275 and 291 had all silently come true** | `CLOSED round-36`. Every one read `BUILT-DORMANT-ON-the owner running it`, some of them for four rounds, and **all six migrations are applied on production**: rounds 27, 28, 29, 34 and 35, probed by their objects and their capability flags on 2026-09-17. Round 29's backfill has visibly run — 44 of 53 games are `settled` and there is no `played` backlog. **This is the second time the re-verification rule has paid for itself** (round 12 found two), and the failure it prevents is exactly the one it found: "still blocked" copied forward is a claim about the past wearing the present's clothes |
| 333 | *Confirmed on production, not just locally.* **`admin_update_game_v2` stores the typed price and always has** | `INFORMATIONAL round-36`. The live catalog agrees with the local finding in row 328: `admin_update_game_v2` contains no `price_for_duration`, while `admin_create_game_v2` and the legacy `admin_update_game` both do. So round 35 v5's derive has been half-applied on production since the day it landed — creates derived, edits did not. **Nothing is owed**, because round 36 wants what the edit path was already doing; it is recorded so the next person reading the round-35 v5 report knows it describes one of the two writers |

| 334 | *Found re-probing row 261, and it is the positive signal that row was missing.* **A checkout session has SETTLED on production** | `CLOSED round-36 — the embedded rail demonstrably works end to end.` `checkout_sessions` now holds **38 `open` and 1 `booked`**. Row 261 could not tell "28 people walked away" from "the rail takes money and nothing settles it", and the owner's explanation ruled out the second; **this is the same conclusion reached from the other direction** — a session that reached `booked` means the webhook fired, `settle_checkout_session` ran and a seat was held. `open` sessions remain inert and expire on their own; the count rising from 28 to 38 is the owner testing, not money accumulating |
| 335 | *Re-probed for the record while closing row 312.* **Every one of the 53 games on production is currently priced on the mapping** | `INFORMATIONAL round-36`. 17 null-duration and 15 sixty-minute games at 150, 14 ninety-minute at 180, and the single 120-minute one at 180 — round 35 v5's reprice, intact. **That census is a snapshot from here on rather than a law**: the first game the owner prices by hand will leave it, which is the feature, and nothing asserts it any more (row 327) |

| 336 | **(R37-1) The measurement: the function ran in Washington and the database is in Ireland** | `SHIPPED round-37`, and it was the verdict the item suspected. `x-vercel-id` read `fra1::iad1` on every production response — served from Frankfurt, **executed in Virginia** — while Supabase is `aws-0-eu-west-1`. A page makes 10-17 database round trips and each crossed the Atlantic twice. **`x-vercel-id` is `<edge>::<function>` and only the second half is where code runs**, which is how this survived thirty-six rounds: the first half said Frankfurt and looked fine |
| 337 | **(R37-2) `"regions": ["dub1"]` — one config line, and the largest single win of the round** | `SHIPPED round-37`. Dublin is Vercel's region in the same city as `eu-west-1`. **Home TTFB 1.404s → 0.371s median** (nine samples, same client, minutes apart), games list 0.500 → 0.230, game detail 0.570 → 0.228. Nothing else changed in that deploy, so it is the one fix this round that is isolated by construction |
| 338 | *What the round-trip count actually was, measured rather than reasoned.* **10 to 17 database calls per navigation, three of them identical on every page** | `SHIPPED round-37`. Counted from the local stack's Kong access log by navigating and diffing it: home 13, games list 14, **game detail 17**, booking 10, account 13. `app_capabilities()`, `site_settings` (the footer's contact row) and `cancellation_refund_cutoff_hours()` fired once per navigation on ALL FIVE journeys, plus two `/auth/v1/user` verifications — one from the proxy, one from the render |
| 339 | **(R37-2) Two waterfalls flattened — home 4 tiers to 2, game detail 6 to 1** | `SHIPPED round-37`. On the game page, roster, venue, waitlist, organizer, own booking and the capability probe need nothing but `game.id` or `game.venue_id`, both already returned by `getGameById`; they were sequential because of the order they were written in over eleven rounds. The genuinely dependent reads — `can_add_guests`, then the credit balance, then the waitlist position — stay in a line. **Measured after the region fix, so the remaining gain was small**: home 0.371 → 0.358, detail 0.228 → 0.213. In the old region the same change would have been worth roughly a second |
| 340 | **(R37-2) Three stable reads cached across requests — bounded, tagged, and deploy-busted** | `SHIPPED round-37`, under the owner's explicit constraint: an applied migration's feature must still appear promptly, never sit behind a stale cache. `lib/db/stableRead.ts` caps them at **sixty seconds**, keys on `VERCEL_DEPLOYMENT_ID` so a deploy busts the lot, tags each so a writer can expire its own (`admin/site`'s save does), and **caches a `false` no longer than a `true`** — there is no negative-result shortcut, so "not applied yet" expires exactly as fast as any other answer. All four properties are asserted in `lib/db/__tests__/stableRead.test.ts`, because raising a TTL is a one-character edit whose cost only appears the next time somebody applies a migration by hand |
| 341 | **(R37-2) The games list served 1.03 MB of images; it now serves 204 KB** | `SHIPPED round-37`. Every photo was the raw upload: venue images stored at 1600x740 painted into a 358x159 card (**one file was 429 KB**), avatars at 512x512 into a 34px circle (51 KB each). Through the optimizer at the sizes actually rendered, measured in a real browser at Pixel 7's viewport and DPR: **1,031,636 → 203,740 bytes, −80%**. Avatars fell from 51 KB to 4.6-9.6 KB. **Measured in a browser rather than by fetching `src`**, because with a `srcset` the attribute names the largest candidate and no phone downloads it |
| 342 | *Three things the image change would have shipped silently.* **A hardcoded `https`, a lazy LCP, and an optimizer that cannot reach the local stack** | `FIXED round-37, all three found by running it.` (a) `protocol: "https"` in `remotePatterns` is correct in production and broke every photo page locally, where storage is `http://127.0.0.1:54321` — the same shape as the `outputFileTracingIncludes` note in CLAUDE.md. (b) `next/image` is LAZY by default and the top card's photo is the Largest Contentful Paint; Next's own console warning is the only thing that catches it, and the first card of the first day now passes `priority`. (c) **The optimizer refuses any upstream resolving to a private IP** — an SSRF guard — so it can never fetch the local stack, and it reports that as `400 "url" parameter is not allowed`, naming the allow-list when the allow-list is not the problem. It is off outside production, and the cost (no spec exercises that path) is written into the config |
| 343 | **(R37-3) The most-tapped control in the product gave no feedback at all** | `SHIPPED round-37`. `e2e/tap-feedback.spec.ts` presses a control and decodes the pixels under the thumb until they move. The bottom nav answered in 40ms and the booking confirm in 26ms; the **game card produced NO VISIBLE CHANGE** until the next page painted. A `.pressable` class now moves brightness and scale — **not a background colour, which is invisible over the card's photograph, and that is exactly where a thumb lands**. Card 64ms on home and 34ms on the list; every one of the six controls is now inside the 100ms budget |
| 344 | **(R37-3) A route without a `loading.tsx` cannot paint anything until the server answers** | `SHIPPED round-37`. `<Link>`'s automatic prefetch on a dynamic route caches only as far as the nearest loading boundary, and `/`, `/account` and `/pass` had none. Measured on production as a live A/B: **`/pass` took 828ms from tap to its first pixel, and 47ms after the boundary was added.** Prefetch itself was already on and working — ten routes prefetched from the games list — which **cannot be seen locally at all, because `next dev` disables prefetching outright** |
| 345 | *The measurement error worth recording, because it pointed the wrong way.* **An A/B that asked its two legs different questions** | `FIXED round-37`. The first version of the boundary A/B asked `/` for "skeleton or real content, whichever is first" and `/pass` for "real content", and reported 55ms against 828ms — a fifteen-fold difference that was partly an artefact of the question. Asked the same question, both land near 50ms and the honest claim is `/pass`'s own before-and-after. **An earlier version was worse**: it waited on `location.pathname` plus "main has some text", which the router satisfies while the PREVIOUS page is still mounted, and read 68ms against 63ms — no difference at all, from an instrument measuring nothing |
| 346 | *What the round did NOT do, with numbers, for the owner to decide.* **Six architectural candidates, deferred rather than half-built** | `OPEN — owner's call`, and each is written up in the round report with its cost and expected gain: (1) one composite RPC per surface to collapse 10-17 round trips to ~3; (2) removing the duplicate `auth.getUser()` by passing the proxy's verified user to the render, which means touching the auth boundary; (3) moving the notification bell off the blocking render path; (4) ISR for `/` and `/games`, which are `force-dynamic` for live capacity and are the largest remaining TTFB item; (5) ~218 KB of gzipped JS per route; (6) a 129 KB venue photograph, which is a source-quality question that touches the crop pipeline and its pixel specs. **None was started**, because the item said not to |
| 347 | *Found by the perf work, not caused by it.* **Two specs were already failing before round 37** | `OPEN — not investigated, and stated rather than absorbed`. `public-profile.spec.ts` "a guest, a shadow and a stranger all 404 the same way" (a shadow player's page answers 200) and `round25.spec.ts` "the checkout page refuses a game it cannot sell" (a checkout opened for a cancelled game). **Confirmed pre-existing by stashing the round's changes and re-running both against a fresh seed**; the second one is the more serious of the two, because it is a guard around money |
| 348 | **The production performance harness, kept** | `SHIPPED round-37`. `npm run perf:prod` — region, TTFB per journey, image bytes in a real browser, tap-to-first-paint, and whether prefetch is on. It is NOT one of the four suites and asserts nothing, because several of these numbers do not exist locally at all: the local stack has no transatlantic hop, `next dev` disables prefetch, and the optimizer is off. `perf/README.md` records what each number means and which instrument mistakes produced wrong ones |

| 349 | *Re-verified, not echoed — and it had come true again.* **Round 36's migration was applied by the owner, so NOTHING is dormant** | `CLOSED round-37`. Row 331 was written four hours earlier saying it was outstanding, and it was, then; probing production before printing it found the `ROUND 36` marker inside `admin_create_game_v2` and the `coalesce(p_price_czk, …)` statement live. **This is the third consecutive round in which re-probing a dormant row changed its answer** — rounds 12, 36 and now 37 — which is the whole argument for the rule. The BUILT-DORMANT list is empty for the first time in this ledger's history |
| 350 | *The correction the round's own rule forced.* **A fix I had written and committed turned out to be worth nothing, measured** | `RECORDED round-37`. `getSessionUser` and `getCurrentPlayer` each called `supabase.auth.getUser()` — a network call that verifies the token rather than trusting the cookie — and the layout, the page and the player read all called those. I wrapped them in `cache()` and wrote a commit claiming four verification round trips saved. **Counting them from the auth container's log said two before and two after**: Next's fetch deduplication was already collapsing the render's calls, and the remaining pair is one from `proxy.ts` and one from the render. The change is kept, because routing `getCurrentPlayer` through `getSessionUser` removes a real redundant path, but it ships as **no measurable win**. The commit message that says otherwise is wrong and this row is the correction |

| 351 | **HOTFIX — the 15- and 20-game passes could not be bought** | `SHIPPED 2026-09-18`, migration written and **handed over** — `20260918110000_pass_tier_ceiling.sql`. **THE DIVERGENCE-CAUSE SENTENCE: the purchase path wrote the tier's price into `credit_topups.amount_czk`, which was capped at 2,000 CZK for hand-entered top-ups, and only the 15- and 20-game tiers are priced above it — so the split between working and failing tiers was exactly the ceiling, not anything about the rows.** All five rows are well-formed and identical in shape: `pass_tiers_credited_rule` holds for every one (credited = games x 180), no column is null, and **the display and the purchase read the same table**. The divergence is that the purchase path applies a second validation the display does not |
| 352 | *What the constraint was actually guarding, which is why it never belonged here.* **50–2000 bounds an amount a PERSON TYPES** | `SHIPPED 2026-09-18`. `create_topup` takes a number from a form and a typo of 20000 must not become a 20,000 CZK obligation. A tier price is not typed by anyone: it comes from `pass_tiers`, a table only an admin writes, already constrained to be positive and not to exceed what it credits. The cap was COPIED onto a path whose input it was never guarding — and `create_pass_topup` said so itself, in a comment that predicted this exact failure: *"The same 50–2000 bounds the ordinary path enforces. A tier priced outside them would be a tier nobody could pay for."* Right about the consequence, wrong about which side should move. The constraint now reads `amount_czk >= min and (pass_games is not null or amount_czk <= max)`: the ceiling applies to hand-entered rows only, and the floor to both |
| 353 | *Three copies of one number, which is how it got out of step.* **The bound is now written down once** | `SHIPPED 2026-09-18`. "2000" lived in the CHECK constraint, in `create_topup` and in `create_pass_topup` — and in the error DETAIL string a player reads, which is a fourth copy and the one that would have gone on saying "between 50 and 2000" for a year after the bound moved. All four now call `public.manual_topup_min_czk()` / `manual_topup_max_czk()`, on the `credit_seat_price_czk()` precedent from round 35 and for the same reason it records: **a constant invites a copy; a function of something else has to be called.** Same stated cost as round 35: Postgres does not re-validate existing rows when an IMMUTABLE function's answer changes |
| 354 | *The part the report understated, found by reading the data rather than the symptom.* **The 20-game pass has NEVER been buyable — not once, since the day it was created** | `FOUND 2026-09-18`. Its original price on 2026-08-02 was **2,300 CZK**, already over the ceiling, so it has been unsellable for its entire six-week existence and `credit_topups` holds **zero** rows for it. The 15-game pass genuinely worked — two confirmed purchases, 2026-08-01 and 2026-08-19, at its old price of 1,875 — and **broke on 2026-09-16 when round 35 v2 repriced it to 2,241**. So the owner's report describes two different bugs wearing one symptom: one regression, and one feature that never worked. **Nothing is stuck and nobody is owed**: the RPC raises before any row is written, so there are no partial or orphaned top-ups for either tier |
| 355 | *Why no round caught it, and the specific shape of the blindness.* **Every pass spec exercised A tier; a per-tier failure is invisible to all of them** | `FIXED 2026-09-18`. Fixtures reach for the cheapest tier, and the cheapest three were under the ceiling. Round 35 v2's own end-state check verified that `/pass` **renders** all five prices and all five discounts on production — which was true, and is exactly the display-versus-purchase divergence: the tiles were never the thing that was broken. Two new guards drive **every row in the table** rather than a list of five numbers, so a sixth tier is covered the day it is inserted: `supabase/tests/pass_tiers_payable.sql` and `e2e/pass-tiers.spec.ts`. **Both were verified to FAIL against the reintroduced bug**, naming the 15 and the 20 individually |
| 356 | *Why the e2e cannot click the button, stated rather than papered over.* **A spec that clicked Purchase would have been just as blind as the ones that existed** | `RECORDED 2026-09-18`. `buyPassAction` checks `embeddedCheckoutEnabled()` first and the E2E environment has no Stripe keys, so a click returns `PASS_NOT_CONFIGURED` before `begin_pass_purchase` is ever called. The spec therefore drives the RPC the button calls, with a real authenticated session, and asserts separately that a tile exists for the same tier — and it checks `czkToMinorUnits` yields a positive integer for every tier, which is the closest this environment gets to session creation. **The gap is real and is named in the spec's own header**, because the next person will otherwise "improve" it into a click |
| 357 | *The half that made it hard to see.* **`AMOUNT_OUT_OF_RANGE` was already being raised and nothing recognised it** | `SHIPPED 2026-09-18`. The code was not in `KNOWN_CODES`, so it fell to `UNKNOWN` and the player read **"Something went wrong. Please try again."** — a message that describes a transient fault and invites the one action guaranteed not to help. It now has its own copy in all four languages, and deliberately does NOT say "try again": *"That amount can't be paid right now. Please tell the organizer."* `PASS_TIER_NOT_FOUND` was recognised for the same reason. **This is not the fix — the migration is** — it is so the next tier priced out of range says what is wrong instead of blaming the network |
