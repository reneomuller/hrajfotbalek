...
| 232 | **Apply `20260912100000_admin_remove_cover.sql`** | `BUILT-DORMANT-ON-the owner running it`. Additive and capability-gated — without it the Remove-banner button's RPC is absent and the control reports the error rather than doing damage. **Shape-only per the round-29 rule**; the behaviour is drilled in `supabase/tests/admin_remove_cover.sql`, which `run.mjs` rolls back |
...
| 241 | **Apply `20260913100000_venue_map_url.sql`** | `BUILT-DORMANT-ON-the owner running it`. Adds `venues.map_url`, **backfills the two venues whose `map_query` holds a URL**, adds the CHECK that stops another, and re-declares both venue writers with the new trailing argument (dropping the old signatures first — row 240). Shape-only verification plus the backfill's own result; behaviour drilled in `supabase/tests/venue_map_url.sql` |
...
| 258 | **Apply round 33's two migrations** | `BUILT-DORMANT-ON-the owner running them`. `20260913110000_player_number_and_rename` (rows 252, 253) and `20260913120000_party_up_to_thirteen` (rows 255, 256). **Both additive, both capability-gated, order does not matter, neither is deploy-first.** Both applied and re-applied against a clean local stack to prove idempotence — the sequence's `setval` is guarded so a second run cannot hand out a number an earlier one already issued. Shape-only verification per the round-29 rule; the behaviour is drilled in `supabase/tests/admin_rename_player.sql` (16 assertions), `player_number.sql` (10) and `party_size.sql` (9), which `run.mjs` rolls back |
...
### Round 33's two migrations (row 258) — additive, order-free, safe to be late
...
### Round 31's migration (row 241) — carries a backfill
...
### Round 30's migration (row 232) — additive, safe to be late
