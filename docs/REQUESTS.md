...
| 216 | **Turn Link and Klarna off in the Stripe dashboard** | `DONE round-36 — CLOSED BY THE OWNER, who confirms the method set is final: card, Apple Pay and Google Pay, with Klarna and the rest deliberately off.` The dashboard step this row waited on has been taken, so it is no longer dormant on anybody. **Round 36 closed the one gap the row could not see**: `payment_method_types: ["card"]` excluded Link as a METHOD and never stopped Checkout presenting it as a WALLET, which the code's own comment claimed it did. `wallet_options.link.display = "never"` now says it in the only parameter that means it |
