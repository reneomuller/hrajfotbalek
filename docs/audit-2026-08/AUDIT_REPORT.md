...
10. **F10 — A missing game returns HTTP 200.** `/game/<any-uuid>` answers with
    a success status, so crawlers index it. The prepared commit fixes the
    *screen* — it is the product's own not-found page now, in three languages,
    instead of a fourth piece of bespoke copy. **The status is still 200 and
    needs your decision**, because `loading.tsx` streams the shell and commits
    the status before anything can throw.

**Prepared on the branch, ready to cherry-pick** — ten commits, each its own,
each message opening with its finding: **F1, F3, F10 (UI half), F4, F5, F17,
F15, F14+F6, F12, F13.** Suites green on the branch with all ten applied: unit
620/620, e2e 291 passed / 0 failed / 4 skipped, lint and `tsc` and `next build`
clean. Nothing is deployed and nothing touches main. See §5 for the table of
SHAs and for why the other seven findings are proposals instead.
...
**Three things in this report are corrections of earlier drafts of itself**,
and they are marked in place: **F14** — half of it was a false positive, my
probe measured the `<input>` when the `<label>` is the hit area; **F1** — it is
not one ruling being broken but two recorded intentions disagreeing, and it
moves to §7; **F10** — the status half cannot be fixed without a decision about
the skeleton. Where the code disagreed with the report, the code won.
...
## 8. Main untouched — verified, not assumed

The brief made this the success criterion, so it is checked mechanically rather
than remembered.

| | |
|---|---|
| `main` at the start of the night | `5565a212c2fae3e0d2200fabcaa8dbdf04ca7208` |
| `main` at the end of the night | `5565a212c2fae3e0d2200fabcaa8dbdf04ca7208` |
| `origin/main` | `5565a212c2fae3e0d2200fabcaa8dbdf04ca7208` |
| `git merge-base main audit/uiux-2026-08` | `5565a212c2fae3e0d2200fabcaa8dbdf04ca7208` |

Three independent facts, because one hash matching is also what a
force-push looks like. The **reflog's newest entry on `main` is still
round 19's** `Round 19: strips regenerated by the final suite` — nothing
was committed to it and nothing was reset. The **merge-base equals main's
tip**, so this branch is a pure descendant: every commit tonight is on the
branch and none is on main. And **`origin/main` agrees**, so nothing was
pushed there either.

Nothing was deployed. No migration was applied. No environment variable was
read or written. Production was browsed and never acted on.
