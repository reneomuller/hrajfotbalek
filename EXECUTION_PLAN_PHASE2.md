...
| 15. Skill badges + organizer display gating | G2 | **Complete** | 2026-08-02 | 2026-08-02 | Migration 29: `game_roster_public` + `photo_path`, shipped with its rendering per §4a. `roster_photo_path.sql` 11/11; 23/23 suites. **Found: `FormatChips` derived the format from `capacity/2`** — removed, contrary to §5.3a. TEST-218/221/232/233 green |
| 15a. Games list rework: compact rows, day picker, View game | G2 | **Complete** | 2026-08-02 | 2026-08-02 | v1.1.4 B, C. Density asserted by counting rows fully inside the Pixel-7 viewport: **6 of 6**. `listUpcomingGames` now returns the instant it decorated with, so the tabs and `hasStarted` cannot disagree across midnight. **Found on the strip: tapping an off-screen day tab reloaded with the selection off-screen** — fixed by progressive enhancement |
| 16. Venue photo (detail only) + share + toasts + header rework | G2 | **Complete** | 2026-08-02 | 2026-08-02 | v1.1.4 A. Opacity moved in the token table only. Toasts travel by redirect, not action state — the CLAUDE.md `revalidatePath` lesson applied rather than rediscovered. **Found: removing the header's Sign up door left `/signup` reachable only by typing the URL** — the login page now carries it, intent forwarded. 75/75 E2E |
| 17. `site_settings` + home rework + community count | G2 | **Complete** | 2026-08-02 | 2026-08-02 | v1.1.4 D. Migration 30, catalog widened in the same migration. `site_settings.sql` 22/22; 24/24 suites. **Found by E2E, not by SQL: PostgREST sends a JS `null` as SQL NULL** — clearing the pick failed and a null count stored silently. Also squared the TS `EventType` union with the CHECK (7 types adrift) |
...
| **🛑 GATE G2 — Games + content** | **G2** | **Verified** | - | 2026-08-04 | Human walk by Oliver. Pass round-trip witnessed end to end (700 received → 750 credited, expiry stamped, receipt correct); batch refund on cancellation confirmed; an ordinary top-up of the same figure left untouched — the v1.1.5 A keying holds in the live path, not only in the SQL suite; admin no-show marking and CSV export working |
| 21. `/football` rewrites + origin-derived surfaces | G3 | **Complete** | 2026-08-07 | 2026-08-07 | Code half only; nothing touches DNS. `/football/*` is an ALIAS — unprefixed stays canonical and internal links stay unprefixed, recorded for the gate rather than assumed. The root redirect is keyed on a literal `hrajsport.cz` so it ships inert and switches itself on at cutover; TEST-228d asserts the inertness rather than trusting it. **Found: REQ-CUT-004 was already satisfied** — every origin-derived surface already resolved through `siteUrl()` / `siteOrigin()`, so this phase proved it instead of building it. 132/132 E2E |
